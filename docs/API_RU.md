...
| --- | --- | --- | --- |
| `image` | string | — | Обязателен. Base64 изображения (префикс `data:image/...;base64,` допускается и вырезается) **или прямая ссылка `http(s)://`** — воркер скачает сам. |
...
`width` и `height` для `edit` **необязательны**. Без них воркер берёт размер
исходника, укладывает его в лимит мегапикселей и округляет вниз до кратного 16:

```text
979x1606  ->  976x1600      (фото с телефона)
4032x3024 -> 1664x1248      (12 МП ужимаются под лимит)
```

Так пропорции ответа совпадают с исходником — это требование модели, обученной
на парах одного размера; при несовпадении правка применяется к части кадра.
Передавать можно только обе стороны сразу, иначе `invalid_input`. Для `generate`
без размеров по-прежнему 1024x1024 — там исходника нет.

### Ссылка вместо base64

`image`, `image_b` и `ref_boost_mask` принимают как base64, так и прямой URL:

```json
{"input": {"action": "edit", "prompt": "change her coat",
           "image": "https://cdn.example.com/photo.jpg"}}
```

Base64 раздувает запрос примерно на треть, ссылка этого не требует. Ограничения
те же: до 20 МБ, только PNG, JPEG или WebP — содержимое проверяется по сигнатуре,
а не по расширению.

Ссылка должна быть **прямой**: редиректы не выполняются, а адреса внутри
инфраструктуры (loopback, приватные диапазоны, link-local) отклоняются с
`invalid_input` — иначе воркером можно было бы дотянуться до чужой сети.

Маска относится к **последнему** референсу: при одной картинке — к ней, при двух — к `image_b`.

### Автоматическая маска по лицу

Рисовать овал руками годится для экспериментов, но не для сервиса, поэтому
воркер умеет находить лицо сам. Передайте `auto_face_mask: true` вместо
`ref_boost_mask`:

```json
{"input": {
  "action": "edit",
  "prompt": "create a photo of this person with her arms crossed",
  "image": "<base64>",
  "auto_face_mask": true,
  "ref_boost": 4.0,
  "grounding_px": 384
...
Детектор — YuNet из OpenCV, он ищет лицо на **последнем** референсе, то есть
там же, куда применяется маска. Если лиц несколько, берётся самое крупное:
обычно это главный субъект кадра. Маска — овал по лицу с запасом в 25%, чтобы
захватить подбородок и линию волос, с мягким краем.

В ответ добавляются два поля:

| Значение `auto_face_mask` | Что произошло |
| --- | --- |
| `detected` | лицо найдено, маска построена; `faces_found` — сколько лиц видел детектор |
| `fallback` | лицо не найдено. Маски нет, а `ref_boost` снижен до **1.75**: при 4.0 модель воспроизвела бы исходник и инструкцию бы проигнорировала |
| `manual` | вместе с флагом пришёл `ref_boost_mask`, и он победил — явная маска всегда важнее |
